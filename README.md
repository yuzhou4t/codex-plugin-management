...
本次从这台 Mac 的用户级目录中核对出 32 个实际 Skill：29 个纳入同步，3 个明确排除。纳入同步的 29 个中，20 个可跨平台自动安装，9 个依赖本机项目路径或 macOS 能力。完整清单见 [`SKILL_INVENTORY.md`](SKILL_INVENTORY.md)。

## 仓库内容

- `skills/`：20 个跨平台 Skill。macOS 与 Windows 安装脚本都会安装。
...
Windows 默认安装 `skills/` 中的 20 个跨平台 Skill，不安装 `skills-macos/`。后者仍保存在仓库中，迁移对应项目并修改其中的绝对路径后可以手动安装。
