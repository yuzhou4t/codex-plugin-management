...
When the requested treatment calls for audio-reactive captions, pre-extract audio data and keep the response subtle enough for readable text. Music does not require audio reactivity by default.
...
```bash
python3 <hyperframes-skill-root>/scripts/extract-audio-data.py audio.mp3 --fps 30 --bands 8 -o audio-data.json
```
