...
Use a transition when the relationship between scenes benefits from a visible handoff. Hard cuts, static openings, entrance animation, exit animation, and end fades are all valid editorial choices. Avoid stacking a separate exit animation under a transition unless the two are intentionally coordinated.
