...
For a long full-route production, independently buildable beats may be delegated when concurrency is available. Keep a small video or tightly coupled sequence in one context. Delegation is an optimization, not a requirement.
...
### 4. Add intentional entrance animations

When an entrance serves the beat, animate from an explicit state to the CSS end state. Use `fromTo()` when deterministic seeking needs both states declared.

### 5. Add mid-scene activity when useful

Persistent motion is optional. Use it to sustain energy or direct attention; preserve intentional stillness and avoid animating every visible element by default.
...
For CSS transition types and their GSAP implementations, read `<hyperframes-skill-root>/references/transitions/catalog.md`.
...
- [ ] Depth and mid-scene activity match the intended treatment rather than a fixed quota
...
For significant animation work, run the animation map from the actual HyperFrames skill root:
...
## Optional Asset Treatments

Choose these when they support the storyboard; a clear static image is also valid:
