...
`lint` checks HTML structure statically — missing attributes, timeline registration, tween conflicts, CSS transform + GSAP conflicts (including inline styles). If `npx hyperframes validate --help` succeeds, also run `npx hyperframes validate` for runtime checks. Do not fail the workflow merely because that optional command is absent from an installed version.

## Visual Verification (snapshot)

After lint and any supported runtime validation pass, capture representative frames when the installed CLI provides a snapshot command:
...
If snapshot is unavailable, use `hyperframes inspect` plus the Studio preview rather than invoking package-internal source paths.
