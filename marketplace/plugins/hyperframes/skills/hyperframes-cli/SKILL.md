...
description: Use the installed HyperFrames CLI to scaffold, lint, inspect, preview, render, transcribe, generate TTS, or troubleshoot a HyperFrames project. Use live command help and doctor output as the source of truth because commands and flags can vary by installed version.
---

# HyperFrames CLI

Everything runs through `npx hyperframes`. The bundled release currently expects Node.js >= 22 and FFmpeg, but verify the installed release before changing the environment:
...
Before using an uncommon command or flag, run `npx hyperframes <command> --help`. The examples below are common workflows, not a frozen command catalog.

## Workflow

1. **Discover** — `npx hyperframes --help`
2. **Scaffold** — `npx hyperframes init my-video`
3. **Write** — author HTML composition (see the `hyperframes` skill)
4. **Lint** — `npx hyperframes lint`
5. **Runtime validate, when supported** — confirm with `npx hyperframes validate --help`
6. **Visual inspect** — `npx hyperframes inspect`
7. **Preview** — `npx hyperframes preview`
8. **Render on request** — `npx hyperframes render`
...
Run `npx hyperframes init --help` to list the templates and automation supported by the installed release. Prefer `init` for a new project when it supports the requested setup.
...
Use this after `lint` and any runtime validation supported by the installed release, especially for compositions with speech bubbles, cards, captions, or tight typography. It reports:
...
Check `npx hyperframes render --help` before selecting format, viewport, quality, worker, Docker, or GPU flags.
...
Run `doctor` first if rendering fails. Common issues include missing FFmpeg, missing Chrome, or low memory.

## Other Commands

Confirm availability with `npx hyperframes --help`:
