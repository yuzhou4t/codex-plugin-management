...
After the user explicitly approves both the preference change and app restart, enable the local API and restart Zotero:
...
2. If `local_api_enabled_pref` is false, explain that enabling it edits Zotero's local preference and that `--restart` quits and relaunches the app. Ask for explicit approval before either action. A request to search, export, cite, or otherwise "operate Zotero" is not permission to change the preference or restart the app.
...
## Preference and Restart Gate

The following commands change desktop state and require an explicit request in the current conversation:

```bash
python3 <plugin-root>/skills/zotero/scripts/zotero.py enable
python3 <plugin-root>/skills/zotero/scripts/zotero.py disable
python3 <plugin-root>/skills/zotero/scripts/zotero.py restart
python3 <plugin-root>/skills/zotero/scripts/zotero.py enable --restart
python3 <plugin-root>/skills/zotero/scripts/zotero.py disable --restart
```

Do not infer this approval from a Zotero search or citation request. If the gate is not approved, stop at the readiness result and report the exact command and state change awaiting approval.
...
# Writes to the Zotero library; confirm first unless explicitly requested
